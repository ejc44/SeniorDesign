...
public class optimalMainEqual {
			...
			if(choice.equals("2")) {
				...
				if(lines.length != ((int) (Math.pow(2,numVars)))) {
					...
					if(cost == -1) {
						...
						network = new BooleanTreeEqual(numVars, table, sumOfProducts, sopTerms);
						...
						network = new BooleanTreeEqual(numVars, linesFromFile);
					}
	...
	********************/
	public static boolean writeDBFile(BooleanTreeEqual network, String indexFilename) {
		try {
